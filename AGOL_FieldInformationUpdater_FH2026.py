...
# 1) Extracts existing alias, description, and value type of fields in a feature service
# containing one or more layers and saves as a lookup table inside a folder.
    ...
    def getValuetype(esri_type):
        ...
        if esri_type in ("esriFieldTypeString", "esriFieldTypeXML"):
            backend_types = [
                ...
                "locationOrPlaceName", "phoneNumber", "emailAddress", "uniqueIdentifier", "dateAndTime"
            ]
                ...
                for AGOL_field in AGOL_layer_fields:
                    ...
                    for sheet_field in sheet_layer_fields:
                        if sheet_field[0] == field_name:
                            ...
                            description = sheet_field[2] if sheet_field[2] else ""
...
#################################### PART 3: DEFINE ALL THE PARAMETERS AND RUN MAIN SCRIPT #####################################

def main():
    ### Define the username, password, and optionally the portal name
    gis = GIS(username="", password="")
    ### Provide the item id
    itemID = ""
    ### Provide the path to the folder that will store the lookup table(s)
    lookupTablepath = r""
    ### Provide a file name for the lookup table. Make sure to include at the end .xlsx
    file_name = ""
